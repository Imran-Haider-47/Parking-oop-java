package parking; // A package in which all files are present
...
public class Main {
	...
	{
	    BufferedWriter writer = new BufferedWriter(   //file write descriptor is opened so that we can write in the file 
	...
	public static void main(String[] args) throws IOException { 
		...
		try {
			...
			e.printStackTrace();  // it is just exception handling 
		...
		case(3):
			...
			if(Pin==12345)// check the pin if valid or not
			{
				...
				switch(option2)  // select one of the above option and check which option user selected and then act accordingly
				{
				case(1): 
					
					System.out.println("Press 1 to add Resident : ");	// option 1
					System.out.println("Press 2 to add Customer : ");   // option 2
					int option3=a.nextInt(); // option menu 
					switch(option3)  // just check which option user has selected 
					{
					case(1):      // if user selects presses  1
						...
						String IssueDate="01/01/2020";  // Date of issue of the permit 
						System.out.println("Enter th name of Resident: "); // to take input the resident name 
						String name=a.nextLine(); // Get the string 
						String Lines=newpermit+","+ IssueDate+","+name;  //since we have to write this in the file so we will combile all the string to makes a larger line 
						Append_At_End(Lines,"Resident.txt"); // Appends the line in the resident file
						Append_At_End(Lines,"Recents.txt");// Appends the line in the recents file to keep track of the recents check ins 
						break;
					case(2):
						...
						Random rand3 = new Random();  // generate ranodm number
				...
				case(3):
					...
					if(resident.Check_Permit(permit2)==true) // checks if permit id is valid or not
					{
						...
						resident.Update_Resi_Checkin(newline);// update the resdient file
						System.exit(0);
					}
					else {
					...
					break;
				case(4):
					cashier.Show_Data("Resident.txt");  // This option is display all the data in the resident.txt file 
					
					break;
				case(5):
					cashier.Show_Data("Recents.txt"); // This option is to display the data of Recents file 
					
					break;
				case(6):
					int Total_sales=cashier.Get_Total_Sale();   // This option is to get the total sales from the cashier class 
					System.out.println("Total_sales ="+Total_sales); // prints the total sales 
					break;
				}
				...
				System.out.println("Invalid Pin ");// if the pin entered by the cashier is incorrect
				System.exit(0);
			}
			break;
		}
